(ns fractal-explorer.core
  (:require [quil.core :as q]
            ...
            [fractal-explorer.modules.generators :as gen]
            [fractal-explorer.modules.effects :as fx]
            [fractal-explorer.modules.modulators :as mod]
            [fractal-explorer.modules.patch :as patch]))
...
(defn setup []
  ...
  (let [w (q/width)
        h (q/height)]
    {:patch (patch/classic-mandelbrot-patch w h)
...
(defn key-pressed [state event]
  (let [patch (:patch state)]
    (case (:key event)
      ;; Preset switching
      \1 (-> state
             (assoc :patch (patch/load-preset :classic-mandelbrot (:width state) (:height state)))
             (assoc :current-preset :classic-mandelbrot))
      
      \2 (-> state
             (assoc :patch (patch/load-preset :psychedelic-julia (:width state) (:height state)))
             (assoc :current-preset :psychedelic-julia))
      
      \3 (-> state
             (assoc :patch (patch/load-preset :trippy-mandelbrot (:width state) (:height state)))
             (assoc :current-preset :trippy-mandelbrot))
      
      \4 (-> state
             (assoc :patch (patch/load-preset :burning-ship-trails (:width state) (:height state)))
             (assoc :current-preset :burning-ship-trails))
      
      \5 (-> state
             (assoc :patch (patch/load-preset :noise-field (:width state) (:height state)))
             (assoc :current-preset :noise-field))
      
      ;; Cycle through presets
      :space (let [presets (keys patch/preset-patches)
               ...
               (-> state
                   (assoc :patch (patch/load-preset next-preset (:width state) (:height state)))
